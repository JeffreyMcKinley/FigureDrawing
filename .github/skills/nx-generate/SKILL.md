...
- List all generators for a plugin: `./nx.bat list @nx/react`
- View available plugins: `./nx.bat list`
...
```bash
./nx.bat g @nx/react:library --help
```
...
```bash
./nx.bat g @nx/react:library --name=my-lib --dry-run --no-interactive
```
