...
1. **Detect the runner:** this workspace has no lockfile and no `node_modules` — use the vendored
   wrapper, `./nx.bat` on Windows and `./nx` elsewhere. (`pnpm nx` / `yarn nx` / `npx nx` all fail
   here.)
